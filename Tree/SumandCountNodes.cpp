...
//find sum of nodes in tree
int printSumOfNodes(TreeNode<int> *root){
        int count=root->data;
        for(int i=0;i<root->children.size();i++){
            count+=printSumOfNodes(root->children[i]);
        }
        return count;
}
...
int main(){
    ...
    printTree(root);
    cout<<endl<<"Number of nodes in tree = "<<countTreeNodes(root)<<endl;
    cout<<endl<<"Sum of nodes in tree = "<<printSumOfNodes(root)<<endl;
